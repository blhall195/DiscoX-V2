...
bool ConfigManager::loadConfig(Config &cfg) {
  ...
  cfg.stabilityTolerance =
      doc["stability_tolerance"] | Defaults::stabilityTolerance;
  cfg.stabilityBufferLength =
      doc["stability_buffer_length"] | (int)Defaults::stabilityBufferLength;
  ...
  cfg.legAngleTolerance =
      doc["leg_angle_tolerance"] | Defaults::legAngleTolerance;
  cfg.cartesianTolerance =
      doc["cartesian_tolerance"] | Defaults::cartesianTolerance;
  cfg.laserDistanceOffset =
      doc["laser_distance_offset"] | Defaults::laserDistanceOffset;
  cfg.calMagConsistency =
      doc["cal_mag_consistency"] | Defaults::calMagConsistency;
  cfg.calGravConsistency =
      doc["cal_grav_consistency"] | Defaults::calGravConsistency;
  cfg.calBufferLength =
      doc["cal_buffer_length"] | (int)Defaults::calBufferLength;
  ...
  cfg.autoShutdownTimeout =
      doc["auto_shutdown_timeout"] | Defaults::autoShutdownTimeout;
  ...
  cfg.screenBrightness =
      doc["screen_brightness"] | (int)Defaults::screenBrightness;
  ...
  const char *userPart =
      (strncmp(rawName, "SAP6_", 5) == 0) ? rawName + 5 : rawName;
...
bool ConfigManager::saveConfig(const Config &cfg) {
  ...
  const char *nameToSave =
      (strncmp(cfg.bleName, "SAP6_", 5) == 0) ? cfg.bleName + 5 : cfg.bleName;
...
bool ConfigManager::loadCalibrationJson(char *buf, size_t bufSize,
                                        size_t &bytesRead) {
  if (!mounted_) {
    return false;
  }
...
bool ConfigManager::saveCalibrationBinary(
    const MagCal::CalibrationBinary &data) {
  if (!mounted_) {
    return false;
  }
...
bool ConfigManager::syncPendingToFlash() {
  ...
  for (uint8_t i = 0; i < pendingBufCount_; i++) {
    int n = snprintf(line, sizeof(line), "%.1f,%.1f,%.2f\n",
                     (double)pendingBuf_[i].az, (double)pendingBuf_[i].inc,
                     (double)pendingBuf_[i].dist);
    size_t written =
        file.write(reinterpret_cast<const uint8_t *>(line), (size_t)n);
    if (written != (size_t)n) {
...
bool ConfigManager::flushPendingReadings(void (*callback)(float, float,
                                                          float)) {
...
void ConfigManager::buildFlagPath(const char *name, char *path,
                                  size_t pathSize) {
