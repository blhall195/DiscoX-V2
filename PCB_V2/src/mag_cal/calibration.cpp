...
Angles Calibration::getAngles(const Eigen::Vector3f &mag,
...
Eigen::Matrix3f
Calibration::getOrientationMatrix(const Eigen::Vector3f &mag,
                                  const Eigen::Vector3f &grav) const {
...
Eigen::Matrix3f Calibration::anglesToMatrix(float azimuth, float inclination,
                                            float roll) {
...
float Calibration::getDip(const Eigen::Vector3f &mag,
                          const Eigen::Vector3f &grav) const {
...
AnomalyType Calibration::checkAnomaly(const Eigen::Vector3f &mag,
...
bool Calibration::fromBinary(const CalibrationBinary &bin) {
  ...
  uint16_t expected =
      crc16_ccitt(reinterpret_cast<const uint8_t *>(&bin), crcLen);
...
Calibration::fitEllipsoid(const std::vector<Eigen::Vector3f> &magData,
...
float Calibration::fitNonLinearQuick(const PairedData &pairedData,
                                     int paramCount) {
  ...
  std::vector<float> params =
      getLstsqNonLinearParams(paramCount, expectedMags, rawMags);
...
float Calibration::accuracy(const PairedData &pairedData) const {
  ...
  for (const auto &pair : pairedData) {
    ...
    // Get orientation vectors for this set (float from runtime path, promote to
    // double)
    std::vector<Eigen::Vector3d> orientations(N);
    for (int i = 0; i < N; i++) {
      orientations[i] =
          getOrientationVector(magSet[i], gravSet[i]).cast<double>();
    }
...
std::pair<float, float>
Calibration::uniformity(const std::vector<Eigen::Vector3f> &magData,
...
Eigen::Vector3f
Calibration::getOrientationVector(const Eigen::Vector3f &mag,
                                  const Eigen::Vector3f &grav) const {
...
Calibration::findSimilarShots(const std::vector<Eigen::Vector3f> &magData,
                              const std::vector<Eigen::Vector3f> &gravData,
  ...
  while (i < N - minRun) {
    ...
    for (int j = N; j >= i + minRun; j--) {
      ...
      std::vector<float> incSub(inclinations.begin() + i,
                                inclinations.begin() + j);
...
std::vector<float> Calibration::getLstsqNonLinearParams(
    int paramCount, const std::vector<Eigen::Vector3f> &expectedMags,
  ...
  for (int i = 0; i < N; i++) {
    Eigen::Vector3d diff =
        expectedMags[i].cast<double>() - rawMags[i].cast<double>();
    ...
    for (int j = 0; j < paramCount; j++) {
      inputData(i * 2 + 1, paramCount + j) =
          gaussBuf[j]; // Z axis → last paramCount cols
    }
...
float Calibration::applyFBCorrection(const float *fwdBearings,
                                     const float *bwdBearings, int numPairs,
                                     float minAmplitude) {
  if (numPairs < 2) {
    return 0.0f;
  }
  ...
  if (H_horiz < 0.1) {
    Serial.println(
        F("FB correction: horizontal field too weak (near magnetic pole?)"));
    return amplitudeF;
  }
  ...
  double delta_north =
      -a * H_horiz * DEG2RAD_D; // convert from degrees to radians
