...
Sensor::Sensor(const char *axesStr)
    : axes_(axesStr), transform_(Eigen::Matrix3f::Identity()),
      centre_(Eigen::Vector3f::Zero()), hasRbfs_(false), fieldAvg_(0.0f),
      fieldStd_(0.0f), calibrated_(false) {}
...
Eigen::Vector3f Sensor::apply(const Eigen::Vector3f &raw) const {
  ...
  // This is row-vector × matrix, equivalent to transform^T * data in
  ...
  if (rbfArr && rbfArr.size() == 3) {
    for (int axis = 0; axis < 3; axis++) {
      ...
      if (axisParams && axisParams.size() > 0) {
        ...
        int count = 0;
        for (size_t j = 0; j < axisParams.size() && count < RBF::MAX_PARAMS;
             j++) {
...
float Sensor::fitEllipsoid(const std::vector<Eigen::Vector3f> &data) {
  ...
  Eigen::Matrix3d transformD =
      eigenVectors * sqrtDiag * eigenVectors.transpose();
...
void Sensor::alignAlongAxis(
    const std::vector<std::vector<Eigen::Vector3f>> &datasets, char axis) {
...
Eigen::Vector3d
Sensor::findPlane(const std::vector<Eigen::Vector3f> &data) const {
...
void Sensor::setExpectedFieldStrengths(
