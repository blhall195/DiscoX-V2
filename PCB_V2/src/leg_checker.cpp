...
CartesianCoordinate::CartesianCoordinate(float x, float y, float z)
    : x(x), y(y), z(z) {}
...
AngularLegChecker::AngularLegChecker(float toleranceDeg)
...
// Constructor only used to set the default on startup
CartesianLegChecker::CartesianLegChecker(float toleranceCm)
...
bool CartesianLegChecker::isValidTolerance(float toleranceCm) {
  return isfinite(toleranceCm) &&
         toleranceCm >= Defaults::cartesianToleranceMin &&
