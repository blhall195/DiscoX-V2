...
bool LDJ100::setLaser(bool on, uint32_t timeoutMs) {
  ...
  return waitForReg(REG_LASER, f, timeoutMs) &&
         f.payload[0] == (on ? 0x0001 : 0x0000);
}
...
const char *LDJ100::statusText(uint16_t status) {
  ...
  case 0x0000:
    return "no error";
  case 0x0001:
    return "input voltage low (needs >= 2.0V)";
  case 0x0002:
    return "internal error (ignorable)";
  case 0x0003:
    return "module temperature too low (<-20C)";
  case 0x0004:
    return "module temperature too high (>+60C)";
  case 0x0005:
    return "target out of range";
  case 0x0006:
    return "invalid measurement result";
  case 0x0007:
    return "ambient light too strong";
  case 0x0008:
  ...
  case 0x000F:
    return "laser signal unstable";
  case 0x0081:
    return "invalid communication format";
  default:
