...
void CalibrationMode::begin(ButtonManager &btns, DisplayManager &disp,
                            DiscoManager &disco, LaserManager &laser,
                            RM3100 &magSensor, SCA3300 &accel,
...
void CalibrationMode::readSensors(Eigen::Vector3f &mag, Eigen::Vector3f &grav) {
  ...
  if (accel_->readAcceleration(gx, gy, gz)) {
    grav =
        Eigen::Vector3f(gx * GRAVITY_MS2, gy * GRAVITY_MS2, gz * GRAVITY_MS2);
  } else {
...
void CalibrationMode::updateFBWaitShot() {
  ...
  if (btns_->wasPressed(Button::UP_DISCO) && fbCount_ >= 2 &&
      !fbHasForesight_) {
