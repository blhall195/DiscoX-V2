...
bool SnakeGame::update() {
  ...
  if (buttons_->wasPressed(Button::MENU) ||
      buttons_->wasPressed(Button::DOWN)) { // exit
    Serial.println(F("Exiting Snake Game..."));
    disco_->turnOff();
    active_ = false;
    return true;
  }
  // (V2: power off is the hardware power button, handled globally)
...
void SnakeGame::drawChar(char ch, int16_t x, int16_t y, uint8_t scale) {
  ...
  for (uint8_t row = 0; row < 7; row++) {
    ...
    for (uint8_t col = 0; col < 5; col++) {
      if (bits & (1 << col)) {
        disp_->fillRect(x + col * scale, y + row * scale, scale, scale,
                        SH110X_WHITE);
      }
    }
  }
