...
static bool sectorRead(uint32_t sector, uint8_t *buf, uint32_t count) {
  ...
  return flash_nrf5x_read(
             buf, FlashLayout::FAT_START + sector * FlashLayout::SECTOR_SIZE,
...
static bool sectorWrite(uint32_t sector, const uint8_t *buf, uint32_t count) {
  ...
  return flash_nrf5x_write(FlashLayout::FAT_START +
                               sector * FlashLayout::SECTOR_SIZE,
                           buf, count * FlashLayout::SECTOR_SIZE) >= 0;
}
...
static int32_t msc_write_cb(uint32_t lba, uint8_t *buffer, uint32_t bufsize) {
  ...
  return sectorWrite(lba, buffer, bufsize / FlashLayout::SECTOR_SIZE)
             ? (int32_t)bufsize
             : -1;
}
...
static bool copyToFat(const char *lfsPath, const char *fatName,
  ...
  UINT written = 0;
  if (optionalHeader) {
    ok = (f_write(&dst, optionalHeader, strlen(optionalHeader), &written) ==
          FR_OK) &&
  ...
  while (ok && haveSrc && src.available()) {
    ...
    ok = (f_write(&dst, s_xferBuf, (UINT)n, &written) == FR_OK) &&
         written == (UINT)n;
  }
...
bool exportFiles(ConfigManager &cfgMgr) {
  ...
  ok &= copyToFat("/pending.txt", "READINGS.CSV",
                  "azimuth_deg,inclination_deg,distance_m\r\n");
...
bool importFiles(ConfigManager &cfgMgr) {
  ...
  if (len > 0) {
    JsonDocument doc;
    if (deserializeJson(doc, fileBuf, (size_t)len) ==
    ...
    Serial.println(
        F("  CONFIG.JSON unreadable/too large — keeping old settings"));
    allValid = false;
  }
  ...
  if (len > 0) {
    ...
    Serial.println(
        F("  CALIBRATION.JSON unreadable/too large — keeping old calibration"));
    allValid = false;
  }
