...
void MenuManager::update(ButtonManager &buttons) {
    ...
    // ── Grace period: ignore inputs briefly after entry so the MENU
    ...
    if (_viewingCalMetrics) {
        if (buttons.wasPressed(Button::FIRE) || buttons.wasPressed(Button::UP_DISCO) ||
            buttons.wasPressed(Button::DOWN) || buttons.wasPressed(Button::MENU)) {
    ...
    // Scroll up
    if (buttons.wasPressed(Button::UP_DISCO)) {
        _root.scroll(-1);
        interacted = true;
    }
    ...
    // Click / select — MENU (B4) or FIRE (B1, the trigger) both select
    if (buttons.wasPressed(Button::MENU) || buttons.wasPressed(Button::FIRE)) {
        _root.click();
        interacted = true;
    }

    // (V2: power off is the hardware power button — no GPIO button here)
...
void MenuManager::buildMenu() {
    ...
    _reformatSub.init(*_display, "Reformat Storage");
    ...
    // ── Reformat submenu (recovery — formats internal LittleFS) ──
    _reformatSub.addAction("No", goToSettings);
    _reformatSub.addAction("Yes - ERASE ALL", reformatFlash);
...
void MenuManager::updateSplaysLabel() {
    snprintf(_splaysLabel, sizeof(_splaysLabel), "Splays: %s", _ctx->config.splaysEnabled ? "On" : "Off");
}
