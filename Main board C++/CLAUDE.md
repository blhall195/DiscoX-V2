...
| F/B field check | **Done** | 18 | Foresight/backsight residual hard-iron correction. Sinusoidal fit + centre adjustment. Menu entry under Enter Calibration → Field Check (F/B). |
...
- **Session 18** (complete): Foresight/backsight residual hard-iron correction. Field testing showed systematic ~2° sinusoidal bearing errors from calibration environment magnetism. **Solution**: Post-calibration "Field Check" mode where user takes 3+ F/B pairs at different bearings. Each pair gives bearing error `e = (fwd - bwd - 180) / 2`. Sinusoidal model `error = a·sin(θ) + b·cos(θ)` solved via least-squares (only 2 unknowns). Correction converted to raw sensor frame: `centre_new = centre_old + inv(transform^T) · δ_calibrated` where δ is derived from a, b and horizontal field strength. **Files changed**: `sensor.h` (added `centreRef()` accessor), `calibration.h/.cpp` (added `applyFBCorrection()` method), `calibration_mode.h/.cpp` (added FB_INTRO/FB_WAIT_FORESIGHT/FB_WAIT_BACKSIGHT/FB_CALCULATING/FB_RESULTS/FB_SAVING states + `beginFBCheck()` entry point), `menu_manager.h/.cpp` (added ENTER_FB_CHECK exit action + "Field Check (F/B)" menu item under Enter Calibration), `main.cpp` (handle ENTER_FB_CHECK transition). **UI flow**: Menu → Enter Calibration → Field Check (F/B) → intro screen → take foresight (MEASURE) → walk to target → take backsight (MEASURE) → repeat → B2 to finish → results screen (amplitude + per-pair errors) → hold B1+B2 save / hold B2 discard. Corrected calibration saved to both JSON and binary flash. Build: RAM 6.3% (12392B), Flash 51.4% (261312B).
