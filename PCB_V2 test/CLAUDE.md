...
`../PCB_V2/`** (ported from the V1 firmware — github.com/blhall195/Mr_Zappy) —
...
1. Driver → `bringup/src/drivers/<ic>.h/.cpp` (class style matches the V1
   firmware — github.com/blhall195/Mr_Zappy, see rm3100.h; verify protocol
   against the datasheet in `hardware/datasheets/`, add it there if missing).
