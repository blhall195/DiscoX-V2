...
// dead element; loudest away from 4 kHz means the wrong resonant part is
// fitted.
//
...
static void runTests() {
  ...
  bool idleLow =
      (digitalRead(PIN_BUZZER_A) == LOW) && (digitalRead(PIN_BUZZER_B) == LOW);
  ...
  Serial.println(
      "then a triple beep. No sound = BUZZ net / rail / dead element.");
