...
static void runTests() {
    ...
    if (sensorOk) {
        snprintf(detail, sizeof(detail), "%lu baud, pins %s%s", (unsigned long)linkBaud,
                 linkSwapped ? "SWAPPED (pins_v2.h wrong?)" : "as pins_v2.h (TX=P1.00 RX=P0.25)",
                 linkHandshake ? ", via 0x55 handshake" : "");
    } else {
        snprintf(detail, sizeof(detail), "no response at any baud, either pin order");
    }
    report("UART link", sensorOk, detail);
    if (!sensorOk) {
        Serial.println("  (laser plugged into U5 JST? LZR_PWR_3V rail is ENA-gated —");
    ...
    snprintf(detail, sizeof(detail), "0x%04X (%s)", status, LDJ100::statusText(status));
    ...
    snprintf(detail, sizeof(detail), "HW 0x%04X, SW 0x%04X, SN 0x%08lX", hwVer, swVer,
    ...
    snprintf(detail, sizeof(detail), "on %s, off %s (red dot for 1.5 s)", onOk ? "ok" : "FAILED",
             offOk ? "ok" : "FAILED");
    ...
    if (ok) {
        snprintf(detail, sizeof(detail), "%lu mm (%.3f m), signal quality %u",
                 (unsigned long)m.distanceMm, m.distanceMm / 1000.0, m.signalQuality);
    } else if (laser.lastStatus() != 0) {
        snprintf(detail, sizeof(detail), "module error %d (%s)", (int16_t)laser.lastStatus(),
                 LDJ100::statusText(laser.lastStatus()));
    } else {
        snprintf(detail, sizeof(detail), "timeout waiting for result frame");
    }
    report("single measurement", ok && m.distanceMm >= 30 && m.distanceMm <= 200000, detail);
    if (!ok && laser.lastStatus() != 0) {
        Serial.println("  (aim the laser at a matte surface 0.03-100 m away, then press");
        Serial.println("   any key to re-run — the module reports out-of-range/weak-signal");
    ...
    Serial.println(failCount == 0 ? ">>> LDJ-100 OK <<<" : ">>> LDJ-100 CHECK FAILED <<<");

    if (sensorOk) {
        laser.startContinuous(LDJ100::CONT_AUTO);
        Serial.println();
        Serial.println("Streaming continuous measurements (distance, signal quality):");
    }
}
...
void loop() {
    ...
    if (laser.poll(m, 500)) {
        lastPrintedError = 0;
        char line[64];
        snprintf(line, sizeof(line), "%8.3f m | SQ %5u", m.distanceMm / 1000.0, m.signalQuality);
        Serial.println(line);
    } else if (laser.lastStatus() != 0 && laser.lastStatus() != lastPrintedError) {
        // Only print error changes — no-target errors stream at ~20 Hz
        lastPrintedError = laser.lastStatus();
        char line[80];
        snprintf(line, sizeof(line), "module error %d (%s)", (int16_t)laser.lastStatus(),
                 LDJ100::statusText(laser.lastStatus()));
        Serial.println(line);
    }
}
