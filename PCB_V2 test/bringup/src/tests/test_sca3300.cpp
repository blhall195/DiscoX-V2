...
static SPIClass scaSpi(NRF_SPIM2, PIN_SCA3300_MISO, PIN_SCA3300_SCK, PIN_SCA3300_MOSI);
static SCA3300 sca;
...
static const char *errorName(SCA3300::Error e) {
    switch (e) {
        case SCA3300::Error::NONE: return "none";
        case SCA3300::Error::CRC: return "CRC mismatch (check wiring/SCK)";
        case SCA3300::Error::STARTUP: return "startup in progress";
        case SCA3300::Error::SENSOR_FLAG: return "sensor error flag (see STATUS)";
        case SCA3300::Error::WHOAMI: return "WHOAMI mismatch";
    }
    return "?";
}
...
static void printStatusBits(uint16_t status) {
    ...
    if (status & SCA3300::STATUS_DIGI1) Serial.println("  DIGI1: digital block error 1");
    if (status & SCA3300::STATUS_DIGI2) Serial.println("  DIGI2: digital block error 2");
    if (status & SCA3300::STATUS_CLK) Serial.println("  CLK: clock error");
    if (status & SCA3300::STATUS_SAT) Serial.println("  SAT: signal saturated");
    if (status & SCA3300::STATUS_TEMP_SAT) Serial.println("  TEMP_SAT: temp saturated");
    if (status & SCA3300::STATUS_PWR) Serial.println("  PWR: startup/voltage flag");
    if (status & SCA3300::STATUS_MEM) Serial.println("  MEM: NVM error");
    if (status & SCA3300::STATUS_PD) Serial.println("  PD: power down");
    if (status & SCA3300::STATUS_MODE_CHANGE) Serial.println("  MODE_CHANGE");
    if (status & SCA3300::STATUS_PIN_CONTINUITY) Serial.println("  PIN_CONTINUITY: internal connection error");
}
...
static void runTests() {
    ...
    snprintf(detail, sizeof(detail), "mode 1 (±3g, 70Hz)%s%s", sensorOk ? "" : ", error: ",
             sensorOk ? "" : errorName(sca.lastError()));
    ...
    if (n > 150) {
        ...
        snprintf(detail, sizeof(detail), "|g| mean %.4f, rms noise %.4f g (%d/200 samples)", mean, rms, n);
    ...
    Serial.println(failCount == 0 ? ">>> SCA3300 OK <<<" : ">>> SCA3300 CHECK FAILED <<<");
}
...
void loop() {
    ...
    if (!sca.readAcceleration(x, y, z)) {
        ...
        uint16_t status;
        if (sca.lastError() == SCA3300::Error::SENSOR_FLAG && sca.readStatus(status)) {
            printStatusBits(status);
        }
        delay(500);
        return;
    ...
    char line[96];
    snprintf(line, sizeof(line), "X %+7.4f  Y %+7.4f  Z %+7.4f g | pitch %+6.1f  roll %+6.1f | %.1f C",
             x, y, z, pitch, roll, temp);
    Serial.println(line);
