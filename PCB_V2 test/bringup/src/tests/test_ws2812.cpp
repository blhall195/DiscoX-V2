...
static Adafruit_NeoPixel pixel(NUM_PIXELS, PIN_NEOPIXEL_DIN,
                               NEO_GRB + NEO_KHZ800);
...
static void runTests() {
  ...
  snprintf(detail, sizeof(detail), "%u pixel on P0.31, GRB @ 800 kHz",
           NUM_PIXELS);
  ...
  Serial.println(
      ">>> WS2812 electrical checks done — CONFIRM COLOURS BY EYE <<<");
  Serial.println();
  Serial.println("Cycling: RED, GREEN, BLUE, WHITE (1 s each), then rainbow.");
  Serial.println(
      "Wrong order = pixel type not GRB; no light = DIN/power issue.");
  Serial.println();
}
...
static bool colourCycle() {
  if (showFor(Adafruit_NeoPixel::Color(255, 0, 0), "RED", 1000))
    return true;
  if (showFor(Adafruit_NeoPixel::Color(0, 255, 0), "GREEN", 1000))
    return true;
  if (showFor(Adafruit_NeoPixel::Color(0, 0, 255), "BLUE", 1000))
    return true;
  if (showFor(Adafruit_NeoPixel::Color(255, 255, 255), "WHITE", 1000))
    return true;
