...
static const char DEVICE_NAME[] = "SAP6_V2_Test"; // SAP6_ prefix: some apps filter on it
...
static const char *phyName(uint8_t phy) {
    switch (phy) {
        case BLE_GAP_PHY_1MBPS: return "1 Mbps";
        case BLE_GAP_PHY_2MBPS: return "2 Mbps";
        case BLE_GAP_PHY_CODED: return "Coded (long range)";
...
static const char *commandName(int cmd) {
    switch (cmd) {
        case SAP6_CMD_ACK0: return "ACK0";
        case SAP6_CMD_ACK1: return "ACK1";
        case SAP6_CMD_STOP_CAL: return "STOP_CAL";
        case SAP6_CMD_START_CAL: return "START_CAL";
        case SAP6_CMD_DEVICE_OFF: return "DEVICE_OFF";
        case SAP6_CMD_LASER_ON: return "LASER_ON";
        case SAP6_CMD_LASER_OFF: return "LASER_OFF";
        case SAP6_CMD_TAKE_SHOT: return "TAKE_SHOT";
        default: return "unknown";
    }
}
...
static void runTests() {
    ...
    if (!bleInited) {
        ...
        report("Bluefruit.begin (event length 24)", ok, ok ? "SoftDevice S140 up" : "begin() failed");
        if (!ok) {
        ...
        snprintf(detail, sizeof(detail), "%u ms (default ~2 s drops at range)", SUP_TIMEOUT_MS);
        ...
        snprintf(detail, sizeof(detail), "\"%s\", SAP6 service UUID in adv data", DEVICE_NAME);
        report("advertising started", ok, detail);
    } else {
        // BLE stack can't re-begin — re-report live state instead
        report("Bluefruit stack", true, "already running (cannot re-init without reset)");
    ...
    Serial.println(">>> Stack checks done — RANGE/PHY NEEDS A PHONE TO CONFIRM <<<");
    Serial.println();
    Serial.print("Connect with nRF Connect (Android) or the survey app to \"");
    Serial.print(DEVICE_NAME);
    Serial.println("\".");
    Serial.println("Expect: 'PHY update request: OK' then 'Coded PHY: YES' (Android w/ BLE 5).");
    Serial.println("iOS has no Coded PHY — '1 Mbps' there is normal, not a failure.");
    Serial.println();
    Serial.println("Buttons: 1=fixed leg  2=random leg  3=burst of 5  4=status dump");
...
static void randomLeg() {
    sendLeg(random(0, 3600) / 10.0f, random(-900, 900) / 10.0f, random(50, 3000) / 100.0f);
}

static void onButton(size_t idx) {
    if (!Bluefruit.connected() && idx < 3) {
        Serial.println("  (not connected — reading will queue and send on connect)");
    }
    switch (idx) {
        case 0: sendLeg(123.4f, -12.3f, 4.56f); break;
        case 1: randomLeg(); break;
        case 2:
...
static void pollConnection() {
    ...
    if (lastConnected && !phyRequestPending && !phyCheckPending &&
        phyWatchdogCount < PHY_WATCHDOG_MAX && millis() - phyWatchdogTime >= PHY_WATCHDOG_MS) {
        phyWatchdogTime = millis();
        BLEConnection *conn = Bluefruit.Connection(Bluefruit.connHandle());
...
void setup() {
    ...
    Serial.println();
    Serial.println("=== Mr Zappy PCB V2 — BLE (SAP6, long-range Coded PHY) test ===");
    Serial.println();
