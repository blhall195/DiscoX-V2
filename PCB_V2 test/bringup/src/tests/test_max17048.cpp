...
static void runTests() {
  ...
  if (!ack) {
    Serial.println("  (is the board powered via the power button? the gauge's "
                   "rail is ENA-gated)");
    return;
  }
  ...
  report("begin / driver init (no reset)", sensorOk,
         "ModelGauge state preserved");
  if (!sensorOk) {
    return;
  }
  ...
  snprintf(detail, sizeof(detail),
           "%.3f V (expect ~3.0-4.2 with a LiPo attached)", v);
  ...
  Serial.println(failCount == 0 ? ">>> MAX17048 OK <<<"
                                : ">>> MAX17048 CHECK FAILED <<<");
  if (pctOk == false) {
    Serial.println(
        "  (SOC out of range on a fresh cell is a ModelGauge seeding");
    Serial.println(
        "   artifact, not a wiring fault — press 'q' to quickStart it)");
  }
}
