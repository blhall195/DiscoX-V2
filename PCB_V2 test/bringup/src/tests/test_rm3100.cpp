...
static void runTests() {
  ...
  if (!ack) {
    Serial.println("  (is the board powered via the power button? RM3100 LDO "
                   "hangs off ENA)");
    return;
  }
  ...
  snprintf(detail, sizeof(detail), "cycle count %u, DRDY on P1.15",
           CYCLE_COUNT);
  ...
  snprintf(detail, sizeof(detail), "raw counts X %ld Y %ld Z %ld", (long)raw.x,
           (long)raw.y, (long)raw.z);
  ...
  snprintf(detail, sizeof(detail),
           "|B| = %.1f uT (expect ~30-60 away from magnets)", magnitude);
  ...
  Serial.println(failCount == 0 ? ">>> RM3100 OK <<<"
                                : ">>> RM3100 CHECK FAILED <<<");
...
void setup() {
  ...
  if (sensorOk) {
    Serial.println();
    Serial.println(
        "Streaming at 10 Hz (x/y/z in uT, magnitude, rough heading):");
  }
}
...
void loop() {
  ...
  snprintf(line, sizeof(line),
           "X %+8.2f  Y %+8.2f  Z %+8.2f uT | |B| %6.2f | heading %5.1f", ux,
           uy, uz, magnitude, heading);
  Serial.println(line);
