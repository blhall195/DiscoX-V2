...
bool RM3100::begin(TwoWire &wire, uint8_t addr, uint16_t cycleCount,
                   int8_t drdyPin) {
  wire_ = &wire;
  addr_ = addr;
...
RM3100::Reading RM3100::getLastReading() {
  ...
  for (uint8_t i = 0; i < 3; i++) {
    uint32_t raw = ((uint32_t)buf[i * 3] << 16) |
                   ((uint32_t)buf[i * 3 + 1] << 8) | (uint32_t)buf[i * 3 + 2];
...
void RM3100::toMicroTesla(const Reading &raw, float &ux, float &uy,
                          float &uz) const {
