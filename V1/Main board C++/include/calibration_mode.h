...
public:
  ...
  void beginFBCheck(ButtonManager &btns, DisplayManager &disp,
                    DiscoManager &disco, LaserEgismos &laser, RM3100 &magSensor,
                    Adafruit_ISM330DHCX &imu, ConfigManager &cfgMgr,
  ...
  bool isActive() const {
    return state_ != CalibState::INACTIVE && state_ != CalibState::DONE;
  }
...
private:
  ...
  CalibState preShutdownState_ =
      CalibState::INACTIVE; // state to restore on cancel
  uint32_t shutdownHoldStart_ =
      0; // millis() when SHUTDOWN hold began (0 = not held)
  ...
  float calEmaAlpha_ =
      Defaults::calEmaAlpha; // lower = smoother (0.3 = ~3 sample lag)
  ...
  uint32_t settleStart_ =
      0; // millis() when consistency first detected (0 = not settling)
  ...
  uint32_t laserOnTime_ =
      0; // millis() when laser should turn back on (0 = inactive)
  ...
  float fbCurrentBearing_ =
      0.0f; // latest EMA-smoothed bearing for live display
  ...
  bool isConsistent(const Eigen::Vector3f *buffer, int count,
