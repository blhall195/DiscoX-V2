...
constexpr float emaAlphaStable =
    0.05f; // low alpha when stationary (max smoothing)
...
constexpr float gyroFreezeThreshold =
    0.02f; // rad/s — below this gyro magnitude, display freezes
constexpr float gyroMovingThreshold =
    0.05f; // rad/s — above this, EMA uses fast alpha (hand tremor ~0.03-0.05)
constexpr uint16_t gyroSettleMs =
    1500; // ms to keep updating after gyro goes still (EMA settle)
