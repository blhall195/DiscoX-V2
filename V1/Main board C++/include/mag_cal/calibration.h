...
enum class AnomalyType : uint8_t { NONE, MAGNETIC, GRAVITY, DIP };
...
class Calibration {
  public:
    ...
    Eigen::Matrix3f getOrientationMatrix(const Eigen::Vector3f &mag, const Eigen::Vector3f &grav) const;
    ...
    std::pair<float, float> fitEllipsoid(const std::vector<Eigen::Vector3f> &magData,
    ...
    std::pair<float, float> uniformity(const std::vector<Eigen::Vector3f> &magData,
                                       const std::vector<Eigen::Vector3f> &gravData) const;

    /// Store field strengths + dip angle statistics for anomaly detection.
    void setFieldCharacteristics(const std::vector<Eigen::Vector3f> &magData,
                                 const std::vector<Eigen::Vector3f> &gravData);

    /// Find runs of shots pointing in similar directions.
    /// Returns vector of (start, end) index pairs.
    std::vector<std::pair<int, int>> findSimilarShots(const std::vector<Eigen::Vector3f> &magData,
                                                      const std::vector<Eigen::Vector3f> &gravData,
  ...
  private:
    ...
    Eigen::Vector3f getOrientationVector(const Eigen::Vector3f &mag, const Eigen::Vector3f &grav) const;
    ...
    static bool isARun(const std::vector<float> &azimuths, const std::vector<float> &inclinations,
                       float precision);

    /// Get raw and expected mag data for non-linear quick fitting
    void getRawAndExpectedMagData(const std::vector<Eigen::Vector3f> &mag,
    ...
    std::vector<float> getLstsqNonLinearParams(int paramCount,
