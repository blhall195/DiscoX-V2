...
class LaserEgismos {
  public:
    bool begin(HardwareSerial &serial, uint8_t address = 0x01, float timeoutSec = 5.0f);
  ...
  private:
    ...
    size_t buildFrame(uint8_t *buf, uint8_t command, const uint8_t *data = nullptr, size_t dataLen = 0);
    ...
    bool parseFrame(const uint8_t *buf, size_t len, uint8_t &command, uint8_t &address,
    ...
    LaserError sendAndReceive(uint8_t command, const uint8_t *&responseData, size_t &responseLen,
                              const uint8_t *sendData = nullptr, size_t sendLen = 0);

    // Send command and check that response data[0] == 0x01.
    LaserError sendCommandExpectAck(uint8_t command, const uint8_t *data = nullptr, size_t dataLen = 0);

    // Check measurement response data for error strings.
    LaserError checkMeasurementResult(const uint8_t *data, size_t len, int32_t &distanceMm);
