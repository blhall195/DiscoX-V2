...
private:
  ...
  bool parseFrame(const uint8_t *buf, size_t len, uint8_t &command,
                  uint8_t &address, const uint8_t *&dataStart, size_t &dataLen);
  ...
  LaserError sendAndReceive(uint8_t command, const uint8_t *&responseData,
                            size_t &responseLen,
  ...
  // Receive buffer (shared across calls — not reentrant, fine for
  // single-threaded)
