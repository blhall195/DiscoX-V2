...
public:
  ...
  void init(const MagCal::Calibration *cal, float emaAlphaStable,
            float emaAlphaMoving, uint8_t stabilityLen,
            float jumpThreshold = 8.0f);

  /// Feed raw sensor readings; updates angles and EMA.
  /// gyroMoving: true if gyro magnitude exceeds the freeze threshold (use fast
  /// alpha).
  void update(const Eigen::Vector3f &rawMag, const Eigen::Vector3f &rawAccel,
              bool gyroMoving);
