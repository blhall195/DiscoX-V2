...
static uint32_t gyroStillSince =
    0; // millis() when gyro first dropped below threshold
static float anchorAz =
    0.0f; // azimuth when gyro settled — display clamped to ±0.1°
...
static int32_t msc_write_cb(uint32_t lba, uint8_t *buffer, uint32_t bufsize) {
  return s_mscFlash->writeBlocks(lba, buffer, bufsize / 512) ? (int32_t)bufsize
                                                             : -1;
}
...
// ═══════════════════════════════════════════════════════════════════
void setup() {
  // Init laser so it can be turned off immediately. There will still be a very
  // brief flash.
  initLaser();
  ...
  {
    bool earlyFlash =
        configMgr.begin(); // safe before Serial — prints are no-ops
  ...
  if (enterCalibMode && magOk && imuOk && dispOk && laserOk) {
    calMode.begin(buttons, display, disco, laser, mag, imu, configMgr,
...
void loop() {
  ...
  if (menuMgr.exitAction() == MenuExitAction::ENTER_FB_CHECK) {
    ...
    if (magOk && imuOk && dispOk && laserOk && calOk) {
      ...
      calMode.beginFBCheck(buttons, display, disco, laser, mag, imu, configMgr,
                           calibration, ctx.config);
    } else {
      Serial.println(
...
static void readSensorsUpdate(uint32_t now) {
  ...
  if (calOk && magOk && imuOk) {
    ...
    float _gyroMag = sqrtf(lastGyroX * lastGyroX + lastGyroY * lastGyroY +
                           lastGyroZ * lastGyroZ);
...
static void updateDisplay(uint32_t now) {
  ...
  if (!ctx.displayFrozen) {
    ...
    float gyroMag = sqrtf(lastGyroX * lastGyroX + lastGyroY * lastGyroY +
                          lastGyroZ * lastGyroZ);
    ...
    if (moving) {
      ...
      dispAz = wrapTo360(
          anchorAz + fmaxf(-0.1f, fminf(0.1f, wrapTo180(liveAz - anchorAz))));
...
static void initPins() {
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_BTN_SHUTDOWN), onShutdownINT,
                  FALLING);
  pinMode(PIN_BTN_FIRE, INPUT_PULLUP);
