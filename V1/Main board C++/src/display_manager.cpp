...
void DisplayManager::showSplash(bool laserOn, const char *nameSuffix) {
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    for (int16_t x = BODY_CX - BODY_HW; x <= BODY_CX + BODY_HW; x++) {
      ...
      if (prevTy >= 0) {
        if (ty != prevTy) {
          _display.drawLine(x - 1, min(ty, prevTy), x, max(ty, prevTy),
                            SH110X_WHITE);
        }
        if (by != prevBy) {
          _display.drawLine(x - 1, min(by, prevBy), x, max(by, prevBy),
                            SH110X_WHITE);
        }
      }
      prevTy = ty;
...
void DisplayManager::drawBattery(float pct) {
  ...
  _display.fillRect(BAT_X + BAT_W, BAT_Y + (BAT_H - BAT_TIP_H) / 2, BAT_TIP_W,
                    BAT_TIP_H, SH110X_WHITE);
