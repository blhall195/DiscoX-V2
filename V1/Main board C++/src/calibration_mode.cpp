...
void CalibrationMode::begin(ButtonManager &btns, DisplayManager &disp,
                            DiscoManager &disco, LaserEgismos &laser,
                            RM3100 &magSensor, Adafruit_ISM330DHCX &imu,
...
void CalibrationMode::updateCollecting() {
  ...
  if (btns_->wasPressed(Button::CALIB) && !waitingForStable_ &&
      iteration_ > 0) {
    magArray_.pop_back();
    gravArray_.pop_back();
    iteration_--;
...
void CalibrationMode::updateShowResults() {
  ...
  if (b1 && b2) {
    ...
    if (holdCounter_ >= HOLD_TIME) {
      Serial.println(
          F("Calibration discarded — rebooting to restore saved cal."));
      auto &d = disp_->getDisplay();
      d.clearDisplay();
      d.setTextColor(SH110X_WHITE);
      d.setTextSize(2);
...
void CalibrationMode::readSensors(Eigen::Vector3f &mag, Eigen::Vector3f &grav) {
  ...
  grav = Eigen::Vector3f(accel.acceleration.x, accel.acceleration.y,
...
Eigen::Vector3f CalibrationMode::average(const Eigen::Vector3f *buffer,
                                         int count) const {
...
void CalibrationMode::recordPoint(const Eigen::Vector3f &mag,
...
void CalibrationMode::acceptPoint(const Eigen::Vector3f &mag,
                                  const Eigen::Vector3f &grav) {
...
void CalibrationMode::calculateAlignment() {
  ...
  for (const auto &run : runs) {
    std::vector<Eigen::Vector3f> magSub(magArray_.begin() + run.first,
                                        magArray_.begin() + run.second);
    std::vector<Eigen::Vector3f> gravSub(gravArray_.begin() + run.first,
                                         gravArray_.begin() + run.second);
...
void CalibrationMode::beginFBCheck(ButtonManager &btns, DisplayManager &disp,
                                   DiscoManager &disco, LaserEgismos &laser,
                                   RM3100 &magSensor, Adafruit_ISM330DHCX &imu,
    ...
    if (fbLegCount_ >= FB_LEG_LEN) {
      ...
      if (azOk) {
        ...
        if (state_ == CalibState::FB_WAIT_FORESIGHT) {
          ...
          float pairError =
              wrapTo180(fbCurrentFwd_ - finalBearing - 180.0f) / 2.0f;
...
void CalibrationMode::showFBResultsScreen() {
  ...
  for (int i = 0; i < fbCount_ && i < 5; i++) {
    ...
    snprintf(buf, sizeof(buf), "%d: err:%.1f sprd:%.1f", i + 1, (double)err,
             (double)maxSpread);
    d.println(buf);
  }
...
float CalibrationMode::getBearing(const Eigen::Vector3f &mag,
                                  const Eigen::Vector3f &grav) {
...
bool CalibrationMode::saveCalibration() {
  ...
  ConfigManager::CalMetrics metrics = {resultMagAcc_, resultGravAcc_,
                                       resultAccuracy_};
