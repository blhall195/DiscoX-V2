...
bool LaserEgismos::begin(HardwareSerial &serial, uint8_t address, float timeoutSec) {
    _serial = &serial;
    _address = address;
...
size_t LaserEgismos::buildFrame(uint8_t *buf, uint8_t command, const uint8_t *data, size_t dataLen) {
    size_t idx = 0;
    buf[idx++] = FRAME_START;
    buf[idx++] = _address;
    buf[idx++] = command;
...
bool LaserEgismos::parseFrame(const uint8_t *buf, size_t len, uint8_t &command, uint8_t &address,
                              const uint8_t *&dataStart, size_t &dataLen) {
    if (len < 5) {
        return false; // minimum: START + addr + cmd + chk + END
    }
...
LaserError LaserEgismos::sendAndReceive(uint8_t command, const uint8_t *&responseData, size_t &responseLen,
...
LaserError LaserEgismos::sendCommandExpectAck(uint8_t command, const uint8_t *data, size_t dataLen) {
    const uint8_t *respData;
    size_t respLen;
    LaserError err = sendAndReceive(command, respData, respLen, data, dataLen);
    if (err != LaserError::OK) {
        return err;
    }
...
LaserError LaserEgismos::checkMeasurementResult(const uint8_t *data, size_t len, int32_t &distanceMm) {
    // Check for known error strings
    if (len == 6 && memcmp(data, "ERR256", 6) == 0) {
        return LaserError::TOO_BRIGHT;
    }
    if (len == 6 && memcmp(data, "ERR255", 6) == 0) {
        return LaserError::TOO_DIM;
    }
    if (len == 6 && memcmp(data, "ERR204", 6) == 0) {
