...
static void startAdvertising() {
  ...
  Bluefruit.Advertising.restartOnDisconnect(
      false); // we restart manually after bond-clear
...
void connectCallback(uint16_t conn_handle) {
  (void)conn_handle;
  // PHY negotiation handled by patched BLEConnection library (Coded PHY
  // preferred)
}
...
void loop() {
  ...
  if (phyRequestPending && millis() - phyRequestTime >= 500) {
    ...
    if (err == 0) {
      ...
      phyRequestPending = false;
      // Set connection TX power to +8 dBm (Bluefruit.setTxPower only sets
      // advertising)
  ...
  if (phyCheckPending && millis() - phyCheckTime >= 2000) {
    ...
    if (conn) {
      ...
      if (phy == BLE_GAP_PHY_CODED) {
        ...
        const char *phyNames[] = {"Auto", "1 Mbps", "2 Mbps", "unknown",
                                  "Coded"};
